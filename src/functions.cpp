...
void pressureAdjustment(float errorValue,int springDirPin,int springStepPin,int input,int output,int setpoint) {
  ...
  if (errorValue > 0) {                     // Setting the direction for the motor (Current output pressure is greater than the setpoint)
    digitalWrite(springDirPin, LOW);
    digitalWrite(input, LOW);
    digitalWrite(output, HIGH);
    delay(200);
    Serial.println("Reversal");             // unwinds the spring housing for two rotations 
    for (int i = 0; i <= 800; i++) {        // 800 value is based on the step/rev setting 
    ...
    while (errorValue < -1) {               // keeps winding the driver until the error is under the value of 1
      digitalWrite(springStepPin, HIGH);
      delayMicroseconds(500);
      digitalWrite(springStepPin, LOW);
      delayMicroseconds(500);
...
bool shutOffFunction(int spindleDirPin,int spindleStepPin,int input, int output, int shutLimit, int preShutLimit) {
  ...
  if (coarse_tighten == true) {
    ...
    digitalWrite(output, HIGH);
    delay(600);
    for (int j = 0; j < 9; j++) {                  // driver winds the spindle down. Value of 9 based on multiple tests to see number of rotations needed to complete shut off 
      for (int i = 0; i <= 400; i++) {
